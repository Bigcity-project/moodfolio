...
function computeBullBearProbability(data: StockAnalysisResponse): { bull: number; bear: number } {
  ...
  if (range > 0) {
    ...
    // Low in range = more upside potential = bull
    // High in range = less upside = bear
...
export function PhaseVerdict({ stockData, stance, onStanceSelect }: PhaseVerdictProps) {
  ...
  // Balance beam tilt: -30deg (full bear) to +30deg (full bull)
    ...
    <div className="flex flex-col items-center justify-center w-full max-w-2xl mx-auto px-6">
      ...
      >
        ...
        <div className="flex justify-between mt-2 text-xs">
          <span className="text-teal-400">Bullish {bull}%</span>
          <span className="text-orange-400">{bear}% Bearish</span>
        </div>
      </motion.div>
      ...
      {stance && (
        <motion.p
          ...
          animate={{ opacity: 1 }}
        >
          Scroll down to explore strategies
        </motion.p>
      )}
    </div>
  )
